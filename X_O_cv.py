...
def detect_shape(cell):
    ...
    for cnt in contours:
        ...
        if len(approx) == 4 and area > 100:  # Filter noise, detect an "X" (intersecting lines)
            return "X"
        elif len(approx) > 8 and area > 100:  # Detect an "O" (circle-like shape)
            return "O"
...
def main():
    ...
    while True:
        ...
        # Define grid coordinates (hardcoded or dynamically detected)
        h, w, _ = frame.shape
